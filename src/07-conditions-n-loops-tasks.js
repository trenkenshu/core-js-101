...
function isInsideCircle(c, p) {
  ...
  if (Math.sqrt(Math.abs(c.center.x - p.x) ** 2 + Math.abs(c.center.y - p.y) ** 2) >= c.radius) {
    inside = false;
  }
...
function findFirstSingleChar(str) {
  const set = str.split(' ').join('').split('');
  const ans = {};
  for (let i = 0; i < set.length; i += 1) {
    if (typeof ans[set[i]] === 'undefined') {
      ans[set[i]] = 1;
    } else {
      ans[set[i]] += 1;
    }
  }
  for (let i = 0; i < Object.keys(ans).length; i += 1) {
    if (Object.values(ans)[i] === 1) {
      return Object.keys(ans)[i];
    }
  }
  return null;
}
...
function isCreditCardNumber(/* ccn */) {
  ...
  /* const str = String(ccn).split('').slice(0, -1);
  const ans = [];
  const incr = str.length % 2;
  for (let i = 0; i < str.length; i += 1) {
    let temp;
    if ((incr === 0 && (i + 1) % 2 === 0) || (incr === 1 && (i + 1) % 2 === 1)) {
      temp = Math.abs(str[i] * 2 - 9);
    } else {
      temp = str[i];
    }
    ans.push(temp);
  }
  return ans.reduce((acc, cur) => acc + +cur, 0) % 10 === 0; */
}
